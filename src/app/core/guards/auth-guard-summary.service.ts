...
export class AuthGuardSummaryService implements CanActivate {
    ...
    return new Observable<boolean>((observer) => {
      this.appStore
        ...
        .subscribe((shippingInfo: ShippingInfo) => {
          let validFlag = true;          
          if (!shippingInfo?.paymentDetails?.shippingBillTo){
            validFlag = validFlag && false;
          }
          if (!shippingInfo?.paymentDetails?.dutiesTaxesBillTo){
            validFlag = validFlag && false;
          }
          if (!validFlag) {
